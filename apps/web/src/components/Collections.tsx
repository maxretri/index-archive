...
export function Collections({ onOpen }: { onOpen(id: string, name: string): void }) {
  ...
  return <div className="collections-view">
    ...
    <div className="collection-grid">
      {collections.data?.map((collection, index) => <button key={collection.id} className="collection-card" onClick={() => onOpen(collection.id, collection.name)}>
        ...
        <strong>{collection.name}</strong><small>{collection.itemCount} {collection.itemCount === 1 ? "ITEM" : "ITEMS"}</small>
      </button>)}
    </div>
