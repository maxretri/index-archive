...
type Sender = (chatId: number, messageId: number, itemCount: number) => Promise<unknown>;
...
export class MediaGroupReplyBatcher {
  ...
  add(key: string, chatId: number, messageId: number) {
  ...
  private async flush(key: string) {
    ...
    try {
      await this.send(group.chatId, group.firstMessageId, group.itemCount);
    } catch (error) {
      this.onError(error);
    }
  }
